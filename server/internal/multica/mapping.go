...
// MapProject 把拉取面的 Multica 项目映射为快照。纯函数：不落库、不写
// store、不改入参；状态/优先级保留 Multica 原词表（向 infera 词表的翻译
// 语义归消费方，本层不发明对照表）。
...
// MapIssue 把拉取面的 Multica issue 映射为快照。纯函数，约定同 MapProject。
