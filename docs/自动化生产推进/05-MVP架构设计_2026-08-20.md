...
03 §六 的 10 项决策 + 1 项外部前提（A11）尚未正式拍板/落实。本设计按 03 推荐候选作为**设计假设**，并标注每个假设门控的件；假设被推翻时对应件按〔03-§七.3〕回报修订，不影响其余件。
...
| A11 | 外部前提 = 三产品接入信息（非 03 §六 决策；唯一外部前提，〔02-§四.3〕B1） | 执行段真绑小鱼儿、infReview http 直连（模式 A）；未到位件以占位 stub 与既有通道推进（〔02-§五.2.8〕） | 〔03-§五.3.1〕〔02-§四.3〕 |
...
1. **首选模式 B（托管平台回读）**：infera 在 unit_test 通过后 push 交付分支并创建 draft PR；infReview 按其自有触发审查该 PR 并把报告回写托管平台——【未验证假设：infReview 触发范围是否覆盖本仓库/draft PR，访谈未证实，见〔01-§2.7〕；01/02 仅记载小鱼儿链路主动交给 infReview 的情形，未见其对本仓库/draft PR 自有触发的依据】；infera 经 GitHub 原生接口（或 `gh` 命令）读回报告，由**回读适配器**转为 `infera-findings` 呈现在 code_review 门。该模式与小鱼儿团队现行接入同型——"报告本来就会回写到代码托管平台……不需要额外一套集成 API"（〔01-§2.7〕），且 infReview 支持多轮带历史复审（〔01-§2.3〕），与失败回环后增量 push 天然匹配。
...
stateDiagram-v2
  ...
  merged --> verified: 合并后验证通过（阶段后置，D4-B 起）
...
sequenceDiagram
  ...
  GH->>RV: PR 变更（infReview 自有触发；触发范围是否覆盖本仓库/draft PR 属未验证假设，见 §二.3 模式 B 注与 §八 风险 12）
  ...
  INF->>INF: 合并后主干验证（新检出：验证命令 + 复现用例）（阶段后置，D4-B 起）
  alt 验证通过
...
| 12 | 合并后验证 | 无（〔02-§五.3.2〕） | merged 后新检出主干（workdir 30min 清理窗〔02-§二.7〕，不复用旧 workdir），执行项目验证命令 + 复现用例，证据落 artifact 回写 Demand | 丙（依赖 D4 完成定义演进为 B/C 后才可立项；超出 MVP）。注：〔03-§五.2〕甲清单无此项；D4 首期拍板为 A |
...
| 12 | infReview 对本仓库/draft PR 不触发（§二.3 模式 B 前提未验证），报告永不产生，模式 B 空转 | 模式 B 启用前先做触发验证 spike（归入三产品接入信息 A11 前提，随 quick win Q3 回读 spike 一并验证）；验证失败则回退模式 A（http 直连，接入信息到位后）或 local 人工兜底（绑 local 跳过该道，人工即审查员） |

---

## 九、MVP 切片验收口径

本节为架构可演示性检查单；阶段验收以〔04-§二.2〕量化指标为准，两者冲突时以 04 为准。
