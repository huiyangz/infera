...
// checkTaskSync 验证 任务同步存储面（INFERA-79 T02 冻结的契约，内存/pg 共用断言）：
// 外部来源字段 roundtrip + 按外部 ID 的幂等 upsert（重复执行不产生重复行、字段更新生效）。
func checkTaskSync(t *testing.T, s Store) {
	t.Helper()
	ctx := context.Background()

	// --- 项目 upsert：首次 = 插入，回读带映射字段。 ---
	p1 := &Project{Name: "自动闭环", RepoURL: "", DefaultBranch: "main", ExternalProjectID: "01m-prj-1"}
	require.NoError(t, s.UpsertProjectByExternalID(ctx, p1))
	require.NotEmpty(t, p1.ID, "插入后回填内部 ID")
	require.NotNil(t, p1.ExternalSyncedAt, "插入后回填同步时间")
	projects, err := s.ListProjects(ctx)
	require.NoError(t, err)
	require.Len(t, projects, 1)
	require.Equal(t, "01m-prj-1", projects[0].ExternalProjectID)
	require.NotNil(t, projects[0].ExternalSyncedAt)

	// --- 项目 upsert 幂等：同外部 ID 换数据 → 行数不变、字段更新、ID 稳定。 ---
	p2 := &Project{Name: "自动闭环（改名）", ExternalProjectID: "01m-prj-1"}
	require.NoError(t, s.UpsertProjectByExternalID(ctx, p2))
	...
	require.Empty(t, projects[1].ExternalProjectID)
	require.Nil(t, projects[1].ExternalSyncedAt, "非同步项目 synced_at 为 nil")

	// 外部 ID 为空 → ErrInvalid（同步入口必须带外部 ID）。
	require.ErrorIs(t, s.UpsertProjectByExternalID(ctx, &Project{Name: "x"}), ErrInvalid)

	// --- 需求 upsert：首次 = 插入，映射字段 roundtrip。 ---
	d1 := &Delivery{
		ProjectID:        p1.ID,
		Title:            "同步需求A",
		Description:      "来自任务同步的描述",
		Status:           "active",
		ExternalIssueID:  "01m-iss-1",
		ExternalIssueKey: "INFERA-100",
		Assignee:         "小鱼儿",
		Priority:         "high",
	}
	require.NoError(t, s.UpsertDeliveryByExternalID(ctx, d1))
	require.NotEmpty(t, d1.ID)
	require.NotNil(t, d1.ExternalSyncedAt)
	got, err := s.GetDelivery(ctx, d1.ID)
	require.NoError(t, err)
	require.Equal(t, "01m-iss-1", got.ExternalIssueID)
	require.Equal(t, "INFERA-100", got.ExternalIssueKey)
	require.Equal(t, "小鱼儿", got.Assignee)
	require.Equal(t, "high", got.Priority)
	require.NotNil(t, got.ExternalSyncedAt)
	require.Empty(t, got.ParentID, "普通需求 parent 为空")

	// 引用不存在的项目 → ErrNotFound（FK 语义，同 UpsertBinding）。
	require.ErrorIs(t, s.UpsertDeliveryByExternalID(ctx, &Delivery{
		ProjectID: "00000000-0000-0000-0000-000000000000",
		Title:     "孤儿", ExternalIssueID: "01m-iss-x",
	}), ErrNotFound)

	// 外部 ID 为空 → ErrInvalid。
	require.ErrorIs(t, s.UpsertDeliveryByExternalID(ctx, &Delivery{ProjectID: p1.ID, Title: "x"}), ErrInvalid)
	...
	require.NoError(t, s.UpsertDeliveryByExternalID(ctx, d1b))
	...
	child := &Delivery{
		...
		ParentID:        d1.ID,
		Wave:            1,
		ExternalIssueID: "01m-iss-2",
	}
	require.NoError(t, s.UpsertDeliveryByExternalID(ctx, child))
	children, err := s.ListChildDeliveries(ctx, d1.ID)
	require.NoError(t, err)
	require.Len(t, children, 1)
	require.Equal(t, "01m-iss-2", children[0].ExternalIssueID)
	...
	require.NoError(t, s.UpsertDeliveryByExternalID(ctx, child2))
	...
	got, err = s.GetDelivery(ctx, d1.ID)
	require.NoError(t, err)
	require.Equal(t, "01m-iss-1", got.ExternalIssueID, "UpdateDelivery 不冲掉同步映射")
	require.Equal(t, "别人", got.Assignee)
	require.Equal(t, "01m-prj-1", func() string {
		pp, err := s.GetProject(ctx, p1.ID)
		require.NoError(t, err)
		return pp.ExternalProjectID
	}())
...
func TestMemoryTaskSync(t *testing.T) {
	checkTaskSync(t, NewMemory())
}

func TestPgTaskSync(t *testing.T) {
	checkTaskSync(t, testPool(t))
}
