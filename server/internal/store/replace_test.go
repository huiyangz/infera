...
func checkReplaceBindings(t *testing.T, s Store) {
	...
	// 现状：项目 spec/test_gen → a1
	require.NoError(t, s.UpsertBinding(ctx, &PipelineBinding{ProjectID: p.ID, Node: "spec", AgentID: a1.ID}))
	require.NoError(t, s.UpsertBinding(ctx, &PipelineBinding{ProjectID: p.ID, Node: "test_gen", AgentID: a1.ID}))

	// 全量替换：test_gen 不在新集合 → 消失；code_gen 新增
	require.NoError(t, s.ReplaceBindings(ctx, p.ID, map[string]string{"spec": a2.ID, "code_gen": a1.ID}))
	require.Equal(t, map[string]string{"spec": a2.ID, "code_gen": a1.ID}, bindingsByNode(mustList(s, ctx, p.ID)))

	// 半写场景：test_gen 引用不存在的 agent → 报错 + 原状态完全保留。
	// （pg 实现按节点序插入：code_gen/spec 先写成功，test_gen 失败触发整体回滚）
	err := s.ReplaceBindings(ctx, p.ID, map[string]string{
		"spec":     a1.ID,
		"code_gen": a2.ID,
		"test_gen": "00000000-0000-0000-0000-000000000000",
	})
	require.ErrorIs(t, err, ErrNotFound)
	require.Equal(t, map[string]string{"spec": a2.ID, "code_gen": a1.ID}, bindingsByNode(mustList(s, ctx, p.ID)), "失败的替换不得留下半写状态")
