// 需求流转装配（INFERA-11 T07）：multica / github 薄 client、reqservice、
...
func flowConfigured(cfg config.Config) bool {
	return cfg.MulticaServerURL != "" || cfg.MulticaToken != "" || cfg.MulticaWorkspaceID != "" ||
		cfg.MulticaProjectID != "" || cfg.MulticaTechLeadAgentID != "" || cfg.MulticaWorkspaceSlug != ""
}

// assembleFlow 装配需求流转三件套。返回 (nil, nil, nil) 表示未接入。
// 构造顺序刻意 multica → github → poller → reqservice：上游 client 的
...
func assembleFlow(pool *pgxpool.Pool, cfg config.Config) (*reqservice.Service, *gatepoll.Poller, error) {
	...
	mc, err := multica.New(cfg.MulticaServerURL, cfg.MulticaToken, cfg.MulticaWorkspaceID)
	if err != nil {
		return nil, nil, err
	}
	...
	reqSvc, err := reqservice.New(pool, mc, gh, reqservice.Options{
		MulticaProjectID:     cfg.MulticaProjectID,
		TechLeadAgentID:      cfg.MulticaTechLeadAgentID,
		MulticaServerURL:     cfg.MulticaServerURL,
		MulticaWorkspaceSlug: cfg.MulticaWorkspaceSlug,
	})
	if err != nil {
		return nil, nil, err
	}
